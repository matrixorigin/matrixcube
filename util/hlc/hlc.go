...
// Timestamp is the HLC timestamp as defined in pb/hlcpb
type Timestamp = hlcpb.Timestamp
...
func (c *HLCClock) Update(m Timestamp) {
	c.update(m)
}
...
func (c *HLCClock) now() Timestamp {
	...
	if c.mu.ts.PhysicalTime >= newPts {
		...
		c.mu.ts = Timestamp{PhysicalTime: newPts}
	}
