...
func TestScheduleReplicasWithRules(t *testing.T) {
	...
	res := metadata.NewShardWithRWLockFromShard(Shard{Start: []byte("b"), End: []byte("c"), Unique: "abc", RuleGroups: []string{"g1"}})
	err := c.GetProphet().GetClient().AsyncAddShardsWithLeastPeers([]*metadata.ShardWithRWLock{res}, []int{2})
