...
func TestRPCDispatch(t *testing.T) {
	...
	rpc1 := newProxyRPC(config.GetDefaultZapLoggerWithLevel(zapcore.DebugLevel).With(zap.String("sp", "sp1")), addr1, 1024*1024, func(r rpc.Request) error {
	...
	rpc2 := newProxyRPC(config.GetDefaultZapLoggerWithLevel(zapcore.DebugLevel).With(zap.String("sp", "sp2")), addr2, 1024*1024, func(r rpc.Request) error {
		t.Logf("sp2 received")
