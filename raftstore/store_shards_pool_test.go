...
func TestGCAllocating(t *testing.T) {
	...
	p.gcAllocating(s, nil)
	v, err := s.GetJobData(p.job)
	assert.NoError(t, err)
	assert.Empty(t, v)
...
func TestDoAllocLocked(t *testing.T) {
	...
	v, err := p.doAllocLocked(&meta.ShardsPoolAllocCmd{Purpose: []byte("p1")}, s, aware)
	assert.NoError(t, err)
	assert.Empty(t, v)
	assert.Equal(t, 1, len(p.mu.createC))
