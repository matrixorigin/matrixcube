...
func TestGCAllocating(t *testing.T) {
	...
	aware.EXPECT().GetShard(gomock.Eq(uint64(1))).Return(core.NewCachedShard(metapb.NewShard(), nil))
	aware.EXPECT().GetShard(gomock.Eq(uint64(2))).Return(core.NewCachedShard(metapb.NewShard(), nil, core.SetWrittenKeys(1)))
	p.mu.state = 1
	p.mu.pools = metapb.ShardsPool{Pools: make(map[uint64]*metapb.ShardPool)}
