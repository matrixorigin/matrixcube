...
func TestSortedEqual(t *testing.T) {
	...
	res := NewCachedShard(meta, &meta.Replicas()[0])
	...
	for _, tc := range testcases {
		...
		for _, i := range tc.idsA {
			downPeersA = append(downPeersA, metapb.ReplicaStats{Replica: meta.Replicas()[i]})
			pendingPeersA = append(pendingPeersA, meta.Replicas()[i])
		}
		for _, i := range tc.idsB {
			downPeersB = append(downPeersB, metapb.ReplicaStats{Replica: meta.Replicas()[i]})
			pendingPeersB = append(pendingPeersB, meta.Replicas()[i])
		}
...
func TestShardKey(t *testing.T) {
	...
	for _, c := range cases {
		...
		// start key changed
		origin := NewCachedShard(&metadata.ShardWithRWLock{Shard: metapb.Shard{End: []byte(got)}}, nil)
		res := NewCachedShard(&metadata.ShardWithRWLock{Shard: metapb.Shard{Start: []byte(got), End: []byte(got)}}, nil)
		...
		// end key changed
		origin = NewCachedShard(&metadata.ShardWithRWLock{Shard: metapb.Shard{Start: []byte(got)}}, nil)
		res = NewCachedShard(&metadata.ShardWithRWLock{Shard: metapb.Shard{Start: []byte(got), End: []byte(got)}}, nil)
		s = DiffShardKeyInfo(origin, res)
...
func TestSetShard(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		res := NewCachedShard(&metadata.ShardWithRWLock{
			Shard: metapb.Shard{
				ID:       uint64(i + 1),
...
func TestShouldRemoveFromSubTree(t *testing.T) {
	...
	origin := NewCachedShard(&metadata.ShardWithRWLock{
		Shard: metapb.Shard{
			ID:       uint64(2),
			Replicas: []metapb.Replica{peer1, peer2, peer3},
			Start:    []byte(fmt.Sprintf("%20d", 10)),
